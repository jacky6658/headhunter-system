 ...
 * 搜尋 518 職缺
 */
async function search518(options = {}) {
  const {
    keyword = 'AI 工程師',
    location = '',
    minSalary = 0,
  ...
  try {
    // 構建搜尋 URL
    const url = `https://www.518.com.tw/job-index-P-1.html?i=1&am=1&ad=${encodeURIComponent(keyword)}`;
    ...
    await page.waitForSelector('.job-item, .job-list, [class*="job"]', { timeout: 15000 }).catch(() => {});
    ...
    const jobsData = await page.evaluate(() => {
      ...
      jobElements.forEach(el => {
        try {
          const titleEl = el.querySelector('a[href*="job-"], h2, .job-name, .title');
          const companyEl = el.querySelector('.company-name, .corp-name, [class*="company"]');
          const salaryEl = el.querySelector('.salary, [class*="salary"]');
          const locationEl = el.querySelector('.area, .location, [class*="area"]');
...
function exportCSV(data, keyword) {
  ...
  const filename = `518_${keyword.replace(/\s+/g, '_')}_${timestamp}.csv`;
...
async function main() {
  ...
  const results = await search518({ keyword, location, minSalary });
